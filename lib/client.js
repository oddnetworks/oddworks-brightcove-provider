...
class Client {
	...
	constructor(args) {
		...
		if (args.concurrentRequestLimit === 'NaN') {
	...
	getVideoCountByPlaylist(args) {
		...
		if (!accountId || typeof accountId !== 'string') {
			throw new Error('An accountId is required to getVideoCountByPlaylist()');
		}

		if (!playlistId || typeof playlistId !== 'string') {
			throw new Error('An playlistId is required to getVideoCountByPlaylist()');
		}
